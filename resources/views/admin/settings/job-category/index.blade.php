                    ...
                    <div class="card">
                        ...
                        <div class="card-content collapse show">
                            <div class="card-body card-dashboard">
                                <p class="card-text"><a href="{{route('create-view.job-category')}}"
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered zero-configuration">
                                        ...
                                        @foreach($jobCategories as $jobCategory)
                                            <tr>
                                                <td>{{$i}}</td>
                                                <td>{{$jobCategory->value}}</td>
                                                <td>
                                                    @if($jobCategory->status === 'active')
                                                        <span style="color:green"> {{$jobCategory->status}}</span>
                                                    @else
                                                        <span style="color:blue"> {{$jobCategory->status}}</span>
                                                    @endif
                                                ...
                                                <td>
                                                    ...
                                                    @if($jobCategory->status === 'active')
                                                            <a href="{{route('inactive.job-category' , encrypt($jobCategory->id))}}" class="dropdown-item" title="click to make inactive the active value">
                                                                <i class="ft-plus-circle primary"></i> Inactive</a>
                                                    @else
                                                            <a href="{{route('active.job-category' ,  encrypt($jobCategory->id))}}" class="dropdown-item" title="click to make active the inactive value">
                                                                <i class="ft-plus-circle primary"></i> Active</a>
                                                    @endif
                                                            <a href="{{route('update.job-category' ,  encrypt($jobCategory->id))}}" class="dropdown-item" title="Edit the value">
                                                                <i class="ft-trash-2"></i> Edit</a>
                                                            <a href="{{route('delete.job-category' ,  encrypt($jobCategory->id))}}" class="dropdown-item" title="Danger! this action will delete the record from database">
