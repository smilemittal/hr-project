                                        ...
                                        @foreach($jobTypes as $jobType)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    @if($jobType->status === 'active')
                                                            <a href="{{route('inactive-job-type' , encrypt($jobType->id))}}" class="dropdown-item" title="click to make inactive the active value">
                                                                <i class="ft-plus-circle primary"></i> Inactive</a>
                                                    @else
                                                            <a href="{{route('active-job-type' ,  encrypt($jobType->id))}}" class="dropdown-item" title="click to make active the inactive value">
                                                                <i class="ft-plus-circle primary"></i> Active</a>
                                                    @endif
                                                            <a href="{{route('update-job-type' ,  encrypt($jobType->id))}}" class="dropdown-item" title="Edit the value">
                                                                <i class="ft-trash-2"></i> Edit</a>
                                                            <a href="{{route('delete-job-type' ,  encrypt($jobType->id))}}" class="dropdown-item" title="Danger! this action will delete the record from database">
