            ...
            <div class="breadcrumbs-top d-inline-block">
                <div class="breadcrumb-wrapper mr-1">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="#">Period Frequency</a>
                        </li>
                    </ol>
                </div>
            </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-content collapse show">
                            <div class="card-body card-dashboard">
                                <p class="card-text"><a href="{{route('create-view.period-frequency')}}"
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered zero-configuration">
                                        ...
                                        @foreach($periodFrequencies as $periodFrequency)
                                            <tr>
                                                <td>{{$i}}</td>
                                                <td>{{$periodFrequency->value}}</td>
                                                <td>
                                                    @if($periodFrequency->status === 'active')
                                                        <span style="color:green"> {{$periodFrequency->status}}</span>
                                                    @else
                                                        <span style="color:blue"> {{$periodFrequency->status}}</span>
                                                    @endif

                                                </td>
                                                <td>{{$periodFrequency->created_at->diffForHumans()}}</td>
                                                <td>
                                                    <span class="dropdown">
                                                    ...
                                                    @if($periodFrequency->status === 'active')
                                                            <a href="{{route('inactive.period-frequency' , encrypt($periodFrequency->id))}}" class="dropdown-item" title="click to make inactive the active value">
                                                                <i class="ft-plus-circle primary"></i> Inactive</a>
                                                    @else
                                                            <a href="{{route('active.period-frequency' ,  encrypt($periodFrequency->id))}}" class="dropdown-item" title="click to make active the inactive value">
                                                                <i class="ft-plus-circle primary"></i> Active</a>
                                                    @endif
                                                            <a href="{{route('update.period-frequency' ,  encrypt($periodFrequency->id))}}" class="dropdown-item" title="Edit the value">
                                                                <i class="ft-trash-2"></i> Edit</a>
                                                            <a href="{{route('delete.period-frequency' ,  encrypt($periodFrequency->id))}}" class="dropdown-item" title="Danger! this action will delete the record from database">
