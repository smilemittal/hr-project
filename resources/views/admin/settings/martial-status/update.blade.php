            ...
            <div class="breadcrumbs-top d-inline-block">
                <div class="breadcrumb-wrapper mr-1">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="#">Update Martial Status</a>
                        </li>
                    </ol>
                </div>
            </div>
                            ...
                            <div class="card-body card-dashboard">
                                <div class="col-xl-4 col-lg-6 col-md-12">
                                    ...
                                    <div class="card-block">
                                        <div class="card-body">
                                            <form action="{{route('post-update-martial-status' , encrypt($record->id))}}" method="post">
                                                @csrf
                                                <fieldset class="form-group">
                                                    <input type="text" name="martial-status" value="{{$record->value}}"
                                                           placeholder="Write Martial Status" class="form-control"
                                                           id="basicInput">
                                                    @if($errors->has('martial-status'))
                                                        <div class="error" style="color:red">{{$errors->first('martial-status')}}</div>
                                                    @endif
                                                </fieldset>
                                                <fieldset class="form-group">
                                                    <button type="submit" class="btn btn-success">Submit</button>
                                                    <a href="{{route('martial-status-index')}}" class="btn btn-primary">View All</a>
                                                </fieldset>
                                            </form>
                                        </div>
                                    </div>
