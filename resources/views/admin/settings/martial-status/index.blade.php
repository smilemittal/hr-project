            ...
            <div class="breadcrumbs-top d-inline-block">
                <div class="breadcrumb-wrapper mr-1">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="#"> Martial Status</a>
                        </li>
                    </ol>
                </div>
            </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-content collapse show">
                            <div class="card-body card-dashboard">
                                <p class="card-text"><a href="{{route('create-view.martial-status')}}"
                                    ...
                                    <table class="table table-striped table-bordered zero-configuration">
                                        ...
                                        @foreach($martialStatus as $martial)
                                            <tr>
                                                ...
                                                <td>{{$martial->value}}</td>
                                                <td>
                                                    @if($martial->status === 'active')
                                                        <span style="color:green"> {{$martial->status}}</span>
                                                    @else
                                                        <span style="color:blue"> {{$martial->status}}</span>
                                                    @endif

                                                </td>
                                                <td>{{$martial->created_at->diffForHumans()}}</td>
                                                <td>
                                                    <span class="dropdown">
                                                    ...
                                                    @if($martial->status === 'active')
                                                            <a href="{{route('inactive.martial-status' , encrypt($martial->id))}}" class="dropdown-item" title="click to make inactive the active value">
                                                                <i class="ft-plus-circle primary"></i> Inactive</a>
                                                    @else
                                                            <a href="{{route('active.martial-status' ,  encrypt($martial->id))}}" class="dropdown-item" title="click to make active the inactive value">
                                                                <i class="ft-plus-circle primary"></i> Active</a>
                                                    @endif
                                                            <a href="{{route('update.martial-status' ,  encrypt($martial->id))}}" class="dropdown-item" title="Edit the value">
                                                                <i class="ft-trash-2"></i> Edit</a>
                                                            <a href="{{route('delete.martial-status' ,  encrypt($martial->id))}}" class="dropdown-item" title="Danger! this action will delete the record from database">
