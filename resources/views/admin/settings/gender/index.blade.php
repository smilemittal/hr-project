                                    ...
                                    <table class="table table-striped table-bordered zero-configuration">
                                        ...
                                        @foreach($genders as $gender)
                                            <tr>
                                                ...
                                                <td>
                                                    @if($gender->status === 'active')
                                                        <a href="{{route('inactive-gender' , encrypt($gender->id))}}" class="btn btn-warning mr-2"
                                                           title="click to make inactive the active value">Inactive</a>
                                                    @else
                                                        <a href="{{route('active-gender' ,  encrypt($gender->id))}}" class="btn btn-success mr-2"
                                                           title="click to make active the inactive value">Active</a>
                                                    @endif
                                                        <a href="{{route('update-gender' ,  encrypt($gender->id))}}" class="btn btn-dark mr-2" title="update the value">Update</a>
                                                        <a href="{{route('delete-gender' ,  encrypt($gender->id))}}" class="btn btn-danger mr-2" title="Danger! this action will delete the record from database">Delete</a>
                                                </td>
                                            </tr>
                                            @php
                                                $i++;
