            ...
            <div class="breadcrumbs-top d-inline-block">
                <div class="breadcrumb-wrapper mr-1">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="#">Document Type</a>
                        </li>
                    </ol>
                </div>
            </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-content collapse show">
                            <div class="card-body card-dashboard">
                                <p class="card-text"><a href="{{route('create-view.document-type')}}"
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered zero-configuration">
                                        ...
                                        @foreach($documentTypes as $documentType)
                                            <tr>
                                                <td>{{$i}}</td>
                                                <td>{{$documentType->value}}</td>
                                                <td>
                                                    @if($documentType->status === 'active')
                                                        <span style="color:green"> {{$documentType->status}}</span>
                                                    @else
                                                        <span style="color:blue"> {{$documentType->status}}</span>
                                                    @endif

                                                </td>
                                                <td>{{$documentType->created_at->diffForHumans()}}</td>
                                                <td>
                                                     <span class="dropdown">
                                                    ...
                                                    @if($documentType->status === 'active')
                                                            <a href="{{route('inactive.document-type' , encrypt($documentType->id))}}" class="dropdown-item" title="click to make inactive the active value">
                                                                <i class="ft-plus-circle primary"></i> Inactive</a>
                                                    @else
                                                            <a href="{{route('active.document-type' ,  encrypt($documentType->id))}}" class="dropdown-item" title="click to make active the inactive value">
                                                                <i class="ft-plus-circle primary"></i> Active</a>
                                                    @endif
                                                            <a href="{{route('update.document-type' ,  encrypt($documentType->id))}}" class="dropdown-item" title="Edit the value">
                                                                <i class="ft-trash-2"></i> Edit</a>
                                                            <a href="{{route('delete.document-type' ,  encrypt($documentType->id))}}" class="dropdown-item" title="Danger! this action will delete the record from database">
