            ...
            <div class="breadcrumbs-top d-inline-block">
                <div class="breadcrumb-wrapper mr-1">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="#">Contact Tag</a>
                        </li>
                    </ol>
                </div>
            </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-content collapse show">
                            <div class="card-body card-dashboard">
                                <p class="card-text"><a href="{{route('create-view.contact-tag')}}"
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered zero-configuration">
                                        ...
                                        @foreach($contactTags as $contactTag)
                                            <tr>
                                                <td>{{$i}}</td>
                                                <td>{{$contactTag->value}}</td>
                                                <td>
                                                    @if($contactTag->status === 'active')
                                                        <span style="color:green"> {{$contactTag->status}}</span>
                                                    @else
                                                        <span style="color:blue"> {{$contactTag->status}}</span>
                                                    @endif

                                                </td>
                                                <td>{{$contactTag->created_at->diffForHumans()}}</td>
                                                <td>
                                                    <span class="dropdown">
                                                    ...
                                                    @if($contactTag->status === 'active')
                                                            <a href="{{route('inactive.contact-tag' , encrypt($contactTag->id))}}" class="dropdown-item" title="click to make inactive the active value">
                                                                <i class="ft-plus-circle primary"></i> Inactive</a>
                                                    @else
                                                            <a href="{{route('active.contact-tag' ,  encrypt($contactTag->id))}}" class="dropdown-item" title="click to make active the inactive value">
                                                                <i class="ft-plus-circle primary"></i> Active</a>
                                                    @endif
                                                            <a href="{{route('update.contact-tag' ,  encrypt($contactTag->id))}}" class="dropdown-item" title="Edit the value">
                                                                <i class="ft-trash-2"></i> Edit</a>
                                                            <a href="{{route('delete.contact-tag' ,  encrypt($contactTag->id))}}" class="dropdown-item" title="Danger! this action will delete the record from database">
